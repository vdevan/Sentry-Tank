...
void handleCaptive()
{
  ...
  String Page = "";
  String Page1 = "";
  Page += String(ESPSSID) + "</b><br /><span id=\"retVal\"></span></p>"; //Add ESPSSID
  ...
  if (n > 0)
  {
      for (int i = 0; i < n; i++)
      {   
         Page += String(F("<div class=\"inf\"><a href=\"#p\" class=\"n\">")) + WiFi.SSID(i) + F("</a>&nbsp;<span class=\"q") + ((WiFi.encryptionType(i) == WIFI_AUTH_OPEN) ? F("\">") : F(" l\">")) + abs(WiFi.RSSI(i)) + " %";        Page += F("</span></div>");
      }
  }
  else
  ...
  if (bConnect)
        Page1 += F("<button type=\"submit\" class=\"btn\" onclick = \"location.href = \'/control\'\" name=\"LED\" style=\"margin-left:15px;\">Tank Control</button>");
  ...
  strcat(logFile, "Wifi Page (Captive) service Started\r\n"); 
  bCaptive = true;
